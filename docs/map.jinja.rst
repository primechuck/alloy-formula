...
We will see a quick setup to configure the ``grafana-alloy`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``grafana-alloy`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``grafana-alloy`` formulas under ``/srv/salt/grafana-alloy/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/grafana-alloy/parameters/dns:domain/
    ln -s dns:domain /srv/salt/grafana-alloy/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/grafana-alloy/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/grafana-alloy/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``grafana-alloy`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``grafana-alloy`` formulas under ``/srv/salt/grafana-alloy/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/grafana-alloy/parameters/roles
...
We create a configuration for the role ``grafana-alloy/server`` in ``/srv/salt/grafana-alloy/parameters/roles/grafana-alloy/server.yaml``:
...
We create another configuration for the role ``grafana-alloy/client`` in ``/srv/salt/grafana-alloy/parameters/roles/grafana-alloy/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles grafana-alloy/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles grafana-alloy/client
...
.. code-block:: console
    ...
    mv /srv/salt/grafana-alloy/parameters/dns:domain/  '/srv/salt/grafana-alloy/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    {%- from tplroot ~ "/map.jinja" import mapdata as grafana-alloy with context %}

    test-does-nothing-but-display-grafana-alloy-as-json:
      test.nop:
        - name: {{ grafana-alloy | json }}
...
Here is an example based on `template-formula/grafana-alloy/config/file.sls`_:
...
.. code-block:: sls
    ...
    grafana-alloy-config-file-file-managed:
      file.managed:
        ...
        - mode: 644
        - user: root
        - group: {{ grafana-alloy.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
          - sls: {{ sls_package_install }}
        - context:
            grafana-alloy: {{ grafana-alloy | json }}

This ``sls`` file expose a ``grafana-alloy`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ grafana-alloy['winner'] }}
...
.. _template-formula/grafana-alloy/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/grafana-alloy/config/file.sls
