...
We will see a quick setup to configure the ``alloy`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``alloy`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``alloy`` formulas under ``/srv/salt/alloy/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/alloy/parameters/dns:domain/
    ln -s dns:domain /srv/salt/alloy/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/alloy/parameters/dns:domain/example.net.yaml``:
...
Create per role configuration for ``alloy`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``alloy`` formulas under ``/srv/salt/alloy/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/alloy/parameters/roles
...
We create a configuration for the role ``alloy/server`` in ``/srv/salt/alloy/parameters/roles/alloy/server.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles alloy/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles alloy/client
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-alloy-as-json:
      test.nop:
        - name: {{ alloy | json }}
...
This ``sls`` file expose a ``alloy`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ alloy['winner'] }}
